...
\begin{figure}
  ...
  \begin{tikzpicture}
    \begin{umlsystem}[x=4]{API REST}
      ...
      \umlusecase[name=crep, x=3, width=2cm]{Créer\\représentation}
      \umlusecase[name=serv, y=-3.5, width=1.5cm]{Appeler\\service}
      \umlusecase[name=enti, x=3, y=-3.5,, width=1.5cm]{Manipuler\\entité}
      \umlusecase[x=0.5,y=1.5, name=hyper, width=2.3cm]{Accèder à\\ une ressource}
    \end{umlsystem}
    \umlactor{Client HTTP}
    \umlactor[x=11]{Générateur JSON}
...
accessible au moyen d'une interface HTTP de type \emph{ Remote
  Procedure Call} (RPC). L'objectif est donc de passer de ce modèle
RPC à un modèle orienté ressources. Cela correspond au cas
d'utilisation \textbf{Accéder à une ressource} de la figure
\ref{fig:needs}.  Ces accès à des ressources peuvent avoir pour effet
\textbf{d'appeler des service} ou de \textbf{manipuler des entités}.
L'accès aux ressources se fera toujours au travers de représentation
qui pour répondre au standard du Web seront basé sur le format JSON.
La \textbf{Création des représentations} devra donc être délégué à un
générateur JSON. Le fait de \textbf{Manipuler les représentations}
permettera de récupérer des liens vers d'autres ressources permettant
d'effectuer d'autres traitements.
