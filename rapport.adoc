...
OFBiz est un framework Web orienté progiciel de gestion intégré.
Ce framework fournit un ensemble de composants ...

=== Architecture orientée Service

Ofbiz est basé sur une architecture orientée service (SOA).
La notion de moteur de service (Service Engine) est définit par l'interface Java `GenericEngine`.

Un service peut être appelé en lui passant un ensemble de valeur en entrée, il renvoie alors un ensemble de valeurs en sortie.
Il est possible d'exécuter un service soit de manière synchrone soit de manière asynchrone.
Il peut être implémenté dans n'importe quel langage de programmation dans la mesure ou les paramètres et résultats doivent être sérialisable.
Les services peuvent être ordonnancé.
Ils peuvent appeler d'autres services.

On parle d'événements OFBiz pour les requêtes HTTP envoyés au serveur OFBiz, mais aussi d'événement de service qui correspondent à différentes phase de l'exécution d'un service.
Les différentes phases existantes sont "auth", "in-validate", "out-validate", "invoke", "commit", or "return".

L'ensemble des routes sont définit dans un fichier `controller.xml`.
Seule les requêtes `POST` et `GET` sont traités du fait que le client est uniquement prévu pour être un navigateur.
Il n'y a de différences dans le traitement des requêtes.
Peu importe la méthode choisi cela à pour effet d'appeler l'evénement associé.

Problème REST => faire correspondre les retours de service avec les codes d'erreur HTTP.
Hypermédia? correspond à la vue.
...
Currently OFBiz is handling HTTP request via its `controlServlet` class which handles `GET` and `POST` request without distinction.  I like the idea proposed by Ean to use an alternate servlet which will be more aware of the semantics associated with HTTP methods.

While Vert.x seems nice, it

I am exploring the idea to use JAX-RS specification via Apache CXF implementation to provide an `/api` servlet to expose both services and entities.
...
- [[[ofbiz-dev]]] Wong, Jonathon, and Rupert Howell. 'Apache OFBiz Development: The Beginner's Tutorial' . Packt Publishing Ltd, 2008.

- [[[ofbiz-cookbook]]] Hoffman, Ruth. 'Apache OFBiz Cookbook'. Packt Publishing Ltd, 2010.

- [[[jax-rs]]] Pericas-Geertsen, Santiago, and Marek Potociar. 'JAX-RS: Java API for RESTful Web Services'. Oracle Corporation, May 2013 (2013): 1-84.
